    ...
    for sectionname in _ini.sections():
        if sectionname.startswith('terminalconfig_'):            
            retdict[sectionname[15:]] = TerminalConfig(
                ...
                terminaltype = _ini.get(sectionname, 'terminaltype', fallback='multiuser'),
                ...
                max_altitude_m = _ini.get(sectionname, 'max_altitude_m', fallback=150),
                max_altitude_without_observer_m = _ini.get(sectionname, 'max_altitude_without_observer_m', fallback=119),
                max_num_flights = _ini.get(sectionname, 'max_num_flights', fallback=25),
                operatinghourscsv = _ini.get(sectionname, 'operatinghourscsv', fallback='./resources/operatinghours/AUSTRIA_MIN.csv'),
